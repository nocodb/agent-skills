...
All examples below invoke `scripts/nocodb.sh` (relative to this skill's directory) — there is no standalone `nc` binary, and `nc` must not be used as it is the netcat utility. On Windows, substitute `scripts/nocodb.ps1` with the same arguments.
